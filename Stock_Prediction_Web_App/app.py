...
def predict_plot():
    ...
    if request.method == 'GET':
        ...
        stock_symbol = companyname.upper()
        start_date = ReferenceStartPeriod
    ...
    return render_template('dashboard.html',
                           ...
                           volume=None if model.ticker('volume') is None else numerize.numerize(model.ticker('volume'), 2),
                           average_vol=None if model.ticker('averageVolume') is None else numerize.numerize(model.ticker('averageVolume'), 2),
                           market_cap= None if model.ticker('marketCap') is None else numerize.numerize(model.ticker('marketCap'), 2),
                           ...
                           share_float=None if model.ticker('floatShares') is None else numerize.numerize(
                               model.ticker('floatShares'), 2),
